/*
 * switch-arizona-ptt.c - Extcon driver Wolfson Arizona devices
 *
...
struct arizona_extcon_info {
	...
	int ptt_state; /* PTT headset state for Samsung PS-LTE project */
};
...
int arizona_micd_mic_reading(struct arizona_extcon_info *info, int val)
{
	...
	if (val > MICROPHONE_MAX_OHM) {
		...
		dev_info(arizona->dev, "set ptt state true\n");
		info->ptt_state = 1;
		goto done;
	}
...
static irqreturn_t arizona_jackdet(int irq, void *data)
{
	...
	dev_info(arizona->dev, "set ptt state false\n");
	info->ptt_state = 0;
...
#include "../../sound/soc/samsung/jack_arizona_ptt.c"
