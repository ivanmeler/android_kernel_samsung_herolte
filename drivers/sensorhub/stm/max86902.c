...
#define FLICKER_INTERVAL	7
#define FLICKER_DATA_CNT	120
...
static int max86902_awb_flicker_read_data(struct max86900_device_data *device, int *data)
{
	...
	if ( device->flicker_sample_cnt % FLICKER_INTERVAL == 0 && irq_status != 0x40 )
		return 1;
	else if ( device->flicker_sample_cnt % FLICKER_INTERVAL != 0 && irq_status != 0x80 )
		return 1;

	recvData[0] = MAX86902_FIFO_DATA;
	if (device->flicker_sample_cnt % FLICKER_INTERVAL == 0) {/* AWB data read, only read IR data */
		if ((err = max86900_read_reg(device, recvData,
		...
		if (device->awb_sample_cnt++ > 1) {
			*data = (recvData[0] << 16 & 0x30000)
					+ (recvData[1] << 8)
					+ (recvData[2] << 0);
			/* Mode change to Flicker */
			...
			err = max86900_write_reg(device, 0x82, 0x04);
			if (err != 0) {
				pr_err("%s - error initializing 0x82!\n",
					__func__);
				return -EIO;
			...
			/* Fifo configuration - interrupt for 20 samples */
			err = max86900_write_reg(device,
				MAX86902_FIFO_CONFIG, 0x0C);
			if (err != 0) {
				pr_err("%s - error initializing MAX86902_FIFO_CONFIG!\n",
					__func__);
				return -EIO;
			...
			/* 1000Hz, LED_PW=400us, SPO2_ADC_RANGE=2048nA */
			err = max86900_write_reg(device,
				MAX86902_SPO2_CONFIGURATION, 0x17);
			if (err != 0) {
				pr_err("%s - error initializing MAX86902_SPO2_CONFIGURATION!\n",
					__func__);
				return -EIO;
		...
		for (i = 0; i < 20; i++) {
			device->flicker_data[device->flicker_data_cnt++] =
				(recvData[i*NUM_BYTES_PER_SAMPLE] << 16 & 0x30000)
				+ (recvData[i*NUM_BYTES_PER_SAMPLE + 1] << 8)
				+ (recvData[i*NUM_BYTES_PER_SAMPLE + 2] << 0);
		}
		...
		if (device->flicker_sample_cnt % FLICKER_INTERVAL == FLICKER_INTERVAL-1) {
			/* Mode change to AWB */
			err = max86900_write_reg(device, 0xFF, 0x54);
			if (err != 0) {
				pr_err("%s - error init MAX86900_MODE_TEST0!\n",
					__func__);
				return -EIO;
			}
			err = max86900_write_reg(device, 0xFF, 0x4d);
			if (err != 0) {
				pr_err("%s - error init MAX86900_MODE_TEST1!\n",
					__func__);
				return -EIO;
			}
			err = max86900_write_reg(device, 0x82, 0x04);
			...
			/* PW_EN = 0 */
			err = max86900_write_reg(device, 0x8f, 0x01);
			if (err != 0) {
				pr_err("%s - error initializing 0x8f!\n",
					__func__);
				return -EIO;
			...
			err = max86900_write_reg(device, 0xFF, 0x00);
			if (err != 0) {
				pr_err("%s - error init MAX86900_MODE_TEST3!\n",
					__func__);
				return -EIO;
			}
			/* Fifo configuration - Average 4 samples */
			err = max86900_write_reg(device,
				MAX86902_FIFO_CONFIG, 0x4C);
			if (err != 0) {
				pr_err("%s - error initializing MAX86902_FIFO_CONFIG!\n",
					__func__);
				return -EIO;
			}
			/* Interrup AVG one sample */
			err = max86900_write_reg(device,
			...
			/* 1600Hz, LED_PW=200us, SPO2_ADC_RANGE=16384nA */
			err = max86900_write_reg(device,
				MAX86902_SPO2_CONFIGURATION, 0x7A);
			if (err != 0) {
				pr_err("%s - error initializing MAX86902_SPO2_CONFIGURATION!\n",
					__func__);
				return -EIO;
			...
			device->flicker_sample_cnt = 0;
			device->awb_sample_cnt = 0;
		} else
			device->flicker_sample_cnt++;
	}
...
static void max86902_awb_flicker_handler(struct max86900_device_data *data)
{
	...
	if (err == 0) {
		if ( data->flicker_sample_cnt % FLICKER_INTERVAL == 1 ) {
			input_report_rel(data->hrm_input_dev, REL_X,  raw_data + 1); /* IR */
			input_sync(data->hrm_input_dev);
		} else {
			input_report_rel(data->hrm_input_dev, REL_X,  -1); /* Flicker data */
			input_sync(data->hrm_input_dev);
		}
	}
