...
static DEVICE_ATTR(rear_torch_flash3, S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH,
	ktd2692_show, ktd2692_store);
#else
...
static int ktd2692_probe(struct platform_device *pdev)
{
	...
	if (device_create_file(camera_flash_dev, &dev_attr_rear_torch_flash3) < 0) {
		LED_ERROR("failed to create device file, %s\n",
				dev_attr_rear_torch_flash3.attr.name);
	}
#else 
	...
	if (device_create_file(camera_flash_dev, &dev_attr_rear_torch_flash) < 0) {
		LED_ERROR("failed to create device file, %s\n",
...
static int __devexit ktd2692_remove(struct platform_device *pdev)
{
	...
	if (camera_flash_dev) {
		...
		device_remove_file(camera_flash_dev, &dev_attr_rear_torch_flash3);
	}
#else
	if (camera_flash_dev) {
