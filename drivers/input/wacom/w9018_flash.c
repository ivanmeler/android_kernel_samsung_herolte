			   ...
			   u16 cmdreg, u16 datareg)
{
	...
	if (!sFeature) {
		input_err(true, &wac_i2c->client->dev, "%s cannot preserve memory \n", __func__);
		goto out;
	}
...
//	ret = wacom_i2c_master_send(client, sFeature, total, WACOM_FLASH_W9018);
	ret = wacom_i2c_send(wac_i2c, sFeature, total, WACOM_I2C_MODE_BOOT);
	if (ret != total) {
		input_err(true, &wac_i2c->client->dev, "Sending Set_Feature failed sent bytes: %d \n", ret);
		goto err;
	}
...
//	ret = wacom_i2c_master_send(client, gFeature, GFEATURE_SIZE, WACOM_FLASH_W9018);
	ret = wacom_i2c_send(wac_i2c, gFeature, GFEATURE_SIZE, WACOM_I2C_MODE_BOOT);
	if (ret != GFEATURE_SIZE) {
		input_info(true, &wac_i2c->client->dev, "%s Sending Get_Feature failed; sent bytes: %d \n", __func__, ret);
		goto err;
	}

	udelay(delay);

//	ret = wacom_i2c_master_recv(client, recv, (buf_size), WACOM_FLASH_W9018);
	ret = wacom_i2c_recv(wac_i2c, recv, buf_size,WACOM_I2C_MODE_BOOT);
	if (ret != buf_size) {
		input_err(true, &wac_i2c->client->dev, "%s Receiving data failed; recieved bytes: %d \n", __func__, ret);
		goto err;
	}
...
static bool flash_blver_w9018(struct wacom_i2c *wac_i2c, int *blver)
{
	...
	bRet = wacom_i2c_set_feature(wac_i2c, REPORT_ID_1, len, command, COMM_REG, DATA_REG);
	if (!bRet) {
		input_err(true, &wac_i2c->client->dev, "%s failed to set feature1\n", __func__);
		return false;
	}

	bRet = wacom_i2c_get_feature(wac_i2c, REPORT_ID_2, RSP_SIZE, response, COMM_REG, DATA_REG, (10 * 1000));
	if (!bRet) {
		input_err(true, &wac_i2c->client->dev, "%s 2 failed to set feature\n", __func__);
		return false;
	}
...
static bool flash_erase_all(struct wacom_i2c *wac_i2c)
{
	...
	do {
		...
		if ((ret = check_progress(&response[3], (BOOT_RSP_SIZE - 3), ERS_ALL_CMD, ECH)) < 0)
			return false;
...
static bool flash_write_w9018(struct wacom_i2c *wac_i2c, unsigned char *flash_data,
			      unsigned long start_address, unsigned long *max_address)
{
	...
	for (ulAddress = start_address; ulAddress < *max_address; ulAddress += FLASH_BLOCK_SIZE) {
		...
		bRet = flash_write_block_w9018(wac_i2c, flash_data, ulAddress, &command_id, &ECH);
		if(!bRet)
			return false;
		if (ECH_len == 3)
			ECH_len = 0;
		...
		if (ECH_len == 3) {
			for (j = 0; j < 3; j++) {
				do {
					...
					if ((ret = check_progress(&response[3], (BOOT_RSP_SIZE - 3), 0x01, ECH_ARRAY[j])) < 0) {
						input_err(true, &wac_i2c->client->dev, "%s mismatched echo array \n", __func__);
						return false;
					}
				} while (ret == PROCESS_INPROGRESS);
			}
...
int wacom_i2c_flash_w9018(struct wacom_i2c *wac_i2c, unsigned char *fw_data)
{
	...
	if (!flash_blver_w9018(wac_i2c, &iBLVer)) {
		input_err(true, &wac_i2c->client->dev, "%s failed to get Boot Loader version \n", __func__);
	...
	if (!flash_mputype_w9018(wac_i2c, &iMpuType)) {
		input_err(true, &wac_i2c->client->dev, "%s failed to get MPU type \n", __func__);
		return -EXIT_FAIL_GET_MPU_TYPE;
	}
	if (iMpuType != MPU_W9018) {
		input_err(true, &wac_i2c->client->dev, "MPU is not for W9018 : %x \n", iMpuType);
		return -EXIT_FAIL_GET_MPU_TYPE;
	}
	printk("MPU type: %x \n", iMpuType);	
	...
	input_info(true, &wac_i2c->client->dev, "%s erasing the current firmware \n", __func__);
	bRet = flash_erase_all(wac_i2c);
	if (!bRet) {
		input_err(true, &wac_i2c->client->dev, "%s failed to erase the user program \n", __func__);
	...
	bRet = flash_write_w9018(wac_i2c, fw_data, start_address, &max_address);
	if (!bRet) {
		input_err(true, &wac_i2c->client->dev, "%s failed to write firmware \n", __func__);
		result = -EXIT_FAIL_WRITE_FIRMWARE;
		goto fail;
	}	
	
	/*Return to the user mode*/
	input_info(true, &wac_i2c->client->dev, "%s closing the boot mode \n", __func__);
	bRet = flash_end_w9018(wac_i2c);
	if (!bRet) {
		input_err(true, &wac_i2c->client->dev, "%s closing boot mode failed  \n", __func__);
...
int wacom_i2c_flash(struct wacom_i2c *wac_i2c)
{
	...
	if (fw_data == NULL) {
		input_err(true, &wac_i2c->client->dev, "epen:Data is NULL. Exit.\n");
		return -1;
	}
	...
	if (ret < 0) {
		input_err(true, &wac_i2c->client->dev, "epen:%s cannot send flash command \n", __func__);
	} else {
		input_info(true, &wac_i2c->client->dev, "epen:%s pass wacom_flash_cmd \n", __func__);
	}

	ret = flash_query_w9018(wac_i2c);
	if(ret < 0) {
		input_err(true, &wac_i2c->client->dev, "epen:%s Error: cannot send query \n", __func__);
		return -EXIT_FAIL;
goto end_wacom_flash;
	}
	
	ret = wacom_i2c_flash_w9018(wac_i2c, fw_data);
	if (ret < 0) {
		input_err(true, &wac_i2c->client->dev, "epen:%s Error: flash failed \n", __func__);
		return -EXIT_FAIL;
goto end_wacom_flash;
	}
