...
struct max77854_muic_vps_data {
	...
	chgtyp_t			chgtyp;
	max77854_reg_ctrl1_t		control1;
	const char			*vps_name;
	const muic_attached_dev_t	attached_dev;
};

static const struct max77854_muic_vps_data muic_vps_table[] = {
	{
...
static int muic_lookup_vps_table(muic_attached_dev_t new_dev)
{
	...
	for (i = 0; i < ARRAY_SIZE(muic_vps_table); i++) {
		const struct max77854_muic_vps_data *tmp_vps;
...
static bool muic_check_support_dev(struct max77854_muic_data *muic_data,
...
static int max77854_muic_update_reg(struct i2c_client *i2c, const u8 reg,
		const u8 val, const u8 mask, const bool debug_en)
{
	...
	if (before_val ^ new_val) {
		ret = max77854_write_reg(i2c, reg, new_val);
		if (ret < 0)
...
static int write_muic_ctrl_reg(struct max77854_muic_data *muic_data,
				const u8 reg, const u8 val)
{
	return max77854_muic_update_reg(muic_data->i2c, reg, val, 0xff, true);
}

static int com_to_open(struct max77854_muic_data *muic_data)
{
...
static int com_to_usb_ap(struct max77854_muic_data *muic_data)
{
	max77854_reg_ctrl1_t reg_val;
	int ret = 0;
...
static int com_to_usb_cp(struct max77854_muic_data *muic_data)
{
	max77854_reg_ctrl1_t reg_val;
	int ret = 0;
...
static int com_to_uart_ap(struct max77854_muic_data *muic_data)
{
	max77854_reg_ctrl1_t reg_val;
	int ret = 0;
...
static int com_to_uart_cp(struct max77854_muic_data *muic_data)
{
	max77854_reg_ctrl1_t reg_val;
	int ret = 0;
...
#if defined(CONFIG_MUIC_MAX77854_SUPPROT_AUDIO_LINE_OUT_CONTROL)
static int com_to_audio(struct max77854_muic_data *muic_data)
{
	max77854_reg_ctrl1_t reg_val;
	int ret;
...
#endif /* CONFIG_MUIC_MAX77854_SUPPROT_AUDIO_LINE_OUT_CONTROL */

static int write_vps_regs(struct max77854_muic_data *muic_data,
...
static int switch_to_ap_uart(struct max77854_muic_data *muic_data,
						muic_attached_dev_t new_dev)
{
	int ret = 0;
...
static int switch_to_cp_uart(struct max77854_muic_data *muic_data,
						muic_attached_dev_t new_dev)
{
	int ret = 0;
...
static int max77854_muic_enable_chgdet(struct max77854_muic_data *muic_data)
{
	return max77854_muic_update_reg(muic_data->i2c, MAX77854_MUIC_REG_CDETCTRL1,
				(MAX77854_ENABLE_BIT << CHGDETEN_SHIFT),
				CHGDETEN_MASK, true);
}

static int max77854_muic_disable_chgdet(struct max77854_muic_data *muic_data)
{
	return max77854_muic_update_reg(muic_data->i2c, MAX77854_MUIC_REG_CDETCTRL1,
				(MAX77854_DISABLE_BIT << CHGDETEN_SHIFT),
				CHGDETEN_MASK, true);
}

static int max77854_muic_enable_accdet(struct max77854_muic_data *muic_data)
{
	return max77854_muic_update_reg(muic_data->i2c, MAX77854_MUIC_REG_CTRL2,
				(MAX77854_ENABLE_BIT << CTRL2_ACCDET_SHIFT),
				CTRL2_ACCDET_MASK, true);
}

static int max77854_muic_disable_accdet(struct max77854_muic_data *muic_data)
{
	return max77854_muic_update_reg(muic_data->i2c, MAX77854_MUIC_REG_CTRL2,
				(MAX77854_DISABLE_BIT << CTRL2_ACCDET_SHIFT),
				CTRL2_ACCDET_MASK, true);
}

static u8 max77854_muic_get_adc_value(struct max77854_muic_data *muic_data)
{
	u8 status;
	u8 adc = ADC_ERROR;
	int ret;

	ret = max77854_read_reg(muic_data->i2c, MAX77854_MUIC_REG_STATUS1,
			&status);
	if (ret)
...
static ssize_t max77854_muic_show_uart_sel(struct device *dev,
					   struct device_attribute *attr,
					   char *buf)
{
	struct max77854_muic_data *muic_data = dev_get_drvdata(dev);
...
static ssize_t max77854_muic_set_uart_sel(struct device *dev,
					  struct device_attribute *attr,
...
static ssize_t max77854_muic_show_usb_sel(struct device *dev,
					   struct device_attribute *attr,
					   char *buf)
{
	struct max77854_muic_data *muic_data = dev_get_drvdata(dev);
...
static ssize_t max77854_muic_set_usb_sel(struct device *dev,
					  struct device_attribute *attr,
					  const char *buf, size_t count)
{
	struct max77854_muic_data *muic_data = dev_get_drvdata(dev);
...
static ssize_t max77854_muic_show_uart_en(struct device *dev,
					   struct device_attribute *attr,
					   char *buf)
{
	struct max77854_muic_data *muic_data = dev_get_drvdata(dev);
...
static ssize_t max77854_muic_set_uart_en(struct device *dev,
					  struct device_attribute *attr,
					  const char *buf, size_t count)
{
	struct max77854_muic_data *muic_data = dev_get_drvdata(dev);
...
static ssize_t max77854_muic_show_adc(struct device *dev,
				   struct device_attribute *attr,
				   char *buf)
{
	struct max77854_muic_data *muic_data = dev_get_drvdata(dev);
	...
	adc = max77854_muic_get_adc_value(muic_data);
...
static ssize_t max77854_muic_show_usb_state(struct device *dev,
				   struct device_attribute *attr,
				   char *buf)
{
	struct max77854_muic_data *muic_data = dev_get_drvdata(dev);
...
static ssize_t max77854_muic_show_attached_dev(struct device *dev,
				   struct device_attribute *attr,
				   char *buf)
{
	struct max77854_muic_data *muic_data = dev_get_drvdata(dev);
...
#if defined(CONFIG_MUIC_MAX77854_SUPPROT_AUDIO_LINE_OUT_CONTROL)
static ssize_t max77854_muic_show_audio_path(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	...
	ret = max77854_read_reg(muic_data->i2c, MAX77854_MUIC_REG_CTRL1, &val);
	if (ret) {
...
static ssize_t max77854_muic_set_audio_path(struct device *dev,
				    struct device_attribute *attr,
				    const char *buf, size_t count)
{
	struct max77854_muic_data *muic_data = dev_get_drvdata(dev);
...
static ssize_t max77854_muic_show_otg_test(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct max77854_muic_data *muic_data = dev_get_drvdata(dev);
	...
	if (muic_check_support_dev(muic_data, ATTACHED_DEV_OTG_MUIC)) {
		ret = max77854_read_reg(i2c, MAX77854_MUIC_REG_CDETCTRL1, &val);
...
static ssize_t max77854_muic_set_otg_test(struct device *dev,
				    struct device_attribute *attr,
				    const char *buf, size_t count)
{
	struct max77854_muic_data *muic_data = dev_get_drvdata(dev);
	...
	if (muic_check_support_dev(muic_data, ATTACHED_DEV_OTG_MUIC)) {
		...
		if (!strncmp(buf, "0", 1)) {
			...
			ret = max77854_muic_disable_chgdet(muic_data);
			if (ret)
				goto err_chgdet;
			ret = max77854_muic_enable_accdet(muic_data);
			if (ret)
				goto err_accdet;
		} else if (!strncmp(buf, "1", 1)) {
			muic_data->is_otg_test = false;
			ret = max77854_muic_enable_chgdet(muic_data);
			if (ret)
				goto err_chgdet;
		} else {
			pr_warn("%s:%s Wrong command\n", MUIC_DEV_NAME, __func__);
		...
		max77854_read_reg(i2c, MAX77854_MUIC_REG_CDETCTRL1, &val);
...
static ssize_t max77854_muic_show_apo_factory(struct device *dev,
					   struct device_attribute *attr,
					   char *buf)
{
	struct max77854_muic_data *muic_data = dev_get_drvdata(dev);
...
static ssize_t max77854_muic_set_apo_factory(struct device *dev,
					  struct device_attribute *attr,
					  const char *buf, size_t count)
{
#if defined(CONFIG_SEC_FACTORY)
...
#if defined(CONFIG_MUIC_MAX77854_IGNORE_ADCERR_WA) && defined(CONFIG_SEC_FACTORY)
static ssize_t max77854_muic_show_ignore_adcerr(struct device *dev,
				   struct device_attribute *attr,
				   char *buf)
{
	struct max77854_muic_data *muic_data = dev_get_drvdata(dev);
...
static ssize_t max77854_muic_set_ignore_adcerr(struct device *dev,
					  struct device_attribute *attr,
					  const char *buf, size_t count)
{
	struct max77854_muic_data *muic_data = dev_get_drvdata(dev);
...
#endif /* CONFIG_MUIC_MAX77854_IGNORE_ADCERR_WA && CONFIG_SEC_FACTORY */

static void max77854_muic_set_adcdbset
			(struct max77854_muic_data *muic_data, int value)
{
	struct i2c_client *i2c = muic_data->i2c;
	int ret;
	u8 val;
	...
	ret = max77854_muic_update_reg(i2c, MAX77854_MUIC_REG_CTRL4, val,
...
#if defined(CONFIG_HV_MUIC_MAX77854_AFC)
static ssize_t max77854_muic_show_afc_disable(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct max77854_muic_data *muic_data = dev_get_drvdata(dev);
	struct muic_platform_data *pdata = muic_data->pdata;
...
static ssize_t max77854_muic_set_afc_disable(struct device *dev,
				    struct device_attribute *attr,
				    const char *buf, size_t count)
{
	struct max77854_muic_data *muic_data = dev_get_drvdata(dev);
...
static DEVICE_ATTR(usb_sel, 0664, max77854_muic_show_usb_sel,
		max77854_muic_set_usb_sel);
static DEVICE_ATTR(uart_en, 0660, max77854_muic_show_uart_en,
		max77854_muic_set_uart_en);
static DEVICE_ATTR(adc, S_IRUGO, max77854_muic_show_adc, NULL);
static DEVICE_ATTR(usb_state, S_IRUGO, max77854_muic_show_usb_state, NULL);
static DEVICE_ATTR(attached_dev, S_IRUGO, max77854_muic_show_attached_dev, NULL);
#if defined(CONFIG_MUIC_MAX77854_SUPPROT_AUDIO_LINE_OUT_CONTROL)
static DEVICE_ATTR(audio_path, 0664,
		max77854_muic_show_audio_path, max77854_muic_set_audio_path);
#endif /* CONFIG_MUIC_MAX77854_SUPPROT_AUDIO_LINE_OUT_CONTROL */
static DEVICE_ATTR(otg_test, 0664,
		max77854_muic_show_otg_test, max77854_muic_set_otg_test);
static DEVICE_ATTR(apo_factory, 0664,
		max77854_muic_show_apo_factory, max77854_muic_set_apo_factory);
#if defined(CONFIG_MUIC_MAX77854_IGNORE_ADCERR_WA) && defined(CONFIG_SEC_FACTORY)
static DEVICE_ATTR(ignore_adcerr, 0664,
		max77854_muic_show_ignore_adcerr, max77854_muic_set_ignore_adcerr);
#endif /* CONFIG_MUIC_MAX77854_IGNORE_ADCERR_WA && CONFIG_SEC_FACTORY */
#if defined(CONFIG_HV_MUIC_MAX77854_AFC)
static DEVICE_ATTR(afc_disable, 0664,
		max77854_muic_show_afc_disable, max77854_muic_set_afc_disable);
#endif /* CONFIG_HV_MUIC_MAX77854_AFC */

static struct attribute *max77854_muic_attributes[] = {
...
static const struct attribute_group max77854_muic_group = {
	.attrs = max77854_muic_attributes,
};

void max77854_muic_read_register(struct i2c_client *i2c)
{
...
static bool max77854_muic_check_dev_factory_charging(struct max77854_muic_data *muic_data)
{
...
static u8 max77854_muic_get_adcmode(struct max77854_muic_data *muic_data)
{
	struct i2c_client *i2c = muic_data->i2c;
	const u8 reg = MAX77854_MUIC_REG_CTRL4;
	u8 val;
	int ret = 0;
...
static int max77854_muic_set_adcmode(struct max77854_muic_data *muic_data,
		const u8 val)
{
	struct i2c_client *i2c = muic_data->i2c;
	const u8 reg = MAX77854_MUIC_REG_CTRL4;
	...
	ret = max77854_muic_update_reg(i2c, reg, (val << shift), mask, true);
	if (ret)
...
static void max77854_muic_adcmode_switch(struct max77854_muic_data *muic_data,
		const u8 val)
{
	const char *name;
	u8 before_val;
	...
	switch (val) {
	case MAX77854_MUIC_CTRL4_ADCMODE_ALWAYS_ON:
		name = "Always ON";
		break;
	case MAX77854_MUIC_CTRL4_ADCMODE_ALWAYS_ON_1M_MON:
		name = "Always ON + 1Mohm monitor";
		break;
	case MAX77854_MUIC_CTRL4_ADCMODE_ONE_SHOT:
		name = "One Shot + Low lp Disconnect Detect";
		break;
	case MAX77854_MUIC_CTRL4_ADCMODE_2S_PULSE:
	...
	before_val = max77854_muic_get_adcmode(muic_data);
	if (before_val == val) {
	...
	ret = max77854_muic_set_adcmode(muic_data, val);
	if (ret) {
...
#if defined(CONFIG_MUIC_ADCMODE_SWITCH_WA)
	after_val = max77854_muic_get_adcmode(muic_data);
	if (after_val == val) {
...
void max77854_muic_set_adcmode_always(struct max77854_muic_data *muic_data)
{
	const u8 val = MAX77854_MUIC_CTRL4_ADCMODE_ALWAYS_ON;

	max77854_muic_adcmode_switch(muic_data, val);

	return;
}

#if !defined(CONFIG_SEC_FACTORY)
static void max77854_muic_set_adcmode_2s_pulse(struct max77854_muic_data *muic_data)
{
	const u8 val = MAX77854_MUIC_CTRL4_ADCMODE_2S_PULSE;
...
void max77854_muic_set_adcmode_oneshot(struct max77854_muic_data *muic_data)
{
	const u8 val = MAX77854_MUIC_CTRL4_ADCMODE_ONE_SHOT;
...
static int max77854_muic_attach_uart_path(struct max77854_muic_data *muic_data,
					muic_attached_dev_t new_dev)
{
	struct muic_platform_data *pdata = muic_data->pdata;
	int ret = 0;
...
static int max77854_muic_attach_usb_path(struct max77854_muic_data *muic_data,
					muic_attached_dev_t new_dev)
{
	struct muic_platform_data *pdata = muic_data->pdata;
	int ret = 0;
...
static int max77854_muic_handle_detach(struct max77854_muic_data *muic_data)
{
	...
	case ATTACHED_DEV_HMT_MUIC:
		...
		max77854_muic_enable_chgdet(muic_data);
		break;
...
static int max77854_muic_logically_detach(struct max77854_muic_data *muic_data,
						muic_attached_dev_t new_dev)
{
	bool noti = true;
	...
	if (max77854_muic_check_dev_factory_charging(muic_data))
		enable_accdet = false;
	...
	case ATTACHED_DEV_QC_CHARGER_9V_MUIC:
		noti = max77854_muic_check_change_dev_afc_charger(muic_data, new_dev);
		if (noti) {
			max77854_muic_set_afc_ready(muic_data, false);
			muic_data->is_afc_muic_prepare = false;
			max77854_hv_muic_reset_hvcontrol_reg(muic_data);
			cancel_delayed_work(&muic_data->hv_muic_qc_vb_work);
			cancel_delayed_work(&muic_data->hv_muic_mping_miss_wa);
		}
#endif
...
static int max77854_muic_handle_attach(struct max77854_muic_data *muic_data,
		muic_attached_dev_t new_dev)
{
	bool logically_notify = false;
	...
	ret = max77854_muic_logically_detach(muic_data, new_dev);
	if (ret)
	...
	case ATTACHED_DEV_HMT_MUIC:
		...
		max77854_muic_disable_chgdet(muic_data);
		break;
	case ATTACHED_DEV_JIG_UART_OFF_MUIC:
	case ATTACHED_DEV_JIG_UART_OFF_VB_OTG_MUIC:
	case ATTACHED_DEV_JIG_UART_OFF_VB_FG_MUIC:
	...
	case ATTACHED_DEV_TA_MUIC:
		...
		if (muic_data->pdata->afc_disable)
			...
			if (muic_data->is_afc_muic_ready == false)
				max77854_muic_prepare_afc_charger(muic_data);
		}
#endif
		ret = write_vps_regs(muic_data, new_dev);
		break;
	...
	case ATTACHED_DEV_CDP_MUIC:
		...
		break;
	case ATTACHED_DEV_UNOFFICIAL_ID_MUIC:
		max77854_muic_disable_accdet(muic_data);
		goto out_without_noti;
	case ATTACHED_DEV_MHL_MUIC:
		ret = write_vps_regs(muic_data, new_dev);
		break;
...
static bool muic_check_vps_adcerr
			...
			const struct max77854_muic_vps_data *tmp_vps, u8 adcerr)
{
	bool ret = false;
...
static bool muic_check_vps_adc
			(const struct max77854_muic_vps_data *tmp_vps, u8 adc)
{
	bool ret = false;
...
static bool muic_check_vps_vbvolt
			(const struct max77854_muic_vps_data *tmp_vps, u8 vbvolt)
{
	bool ret = false;
...
static bool muic_check_vps_chgdetrun
			(const struct max77854_muic_vps_data *tmp_vps, u8 chgdetrun)
{
	bool ret = false;
...
static bool muic_check_vps_chgtyp
			(const struct max77854_muic_vps_data *tmp_vps, u8 chgtyp)
{
	bool ret = false;
...
static bool muic_check_otg_test(struct max77854_muic_data *muic_data)
{
	bool ret = false;
...
static bool is_need_muic_adcmode_2s_pulse(struct max77854_muic_data *muic_data)
{
	u8 adc = muic_data->status1 & STATUS1_ADC_MASK;
	bool ret = false;
...
static void max77854_muic_detect_dev(struct max77854_muic_data *muic_data, int irq)
{
	struct i2c_client *i2c = muic_data->i2c;
	const struct max77854_muic_vps_data *tmp_vps;
	...
	ret = max77854_bulk_read(i2c, MAX77854_MUIC_REG_STATUS1, 3, status);
	if (ret) {
		pr_err("%s:%s fail to read muic reg(%d)\n", MUIC_DEV_NAME,
				__func__, ret);
		return;
	...
	if (is_need_muic_adcmode_continuous(new_dev)) {
		...
		max77854_muic_set_adcmode_2s_pulse(muic_data);
	} else {
	...
	if (intr == MUIC_INTR_ATTACH) {
		...
		ret = max77854_muic_handle_attach(muic_data, new_dev);
		if (ret)
			pr_err("%s:%s cannot handle attach(%d)\n", MUIC_DEV_NAME,
								__func__, ret);
	} else {
		pr_info("%s:%s DETACHED\n", MUIC_DEV_NAME, __func__);
		ret = max77854_muic_handle_detach(muic_data);
		if (ret)
...
#if defined(CONFIG_MUIC_MAX77854_RESET_WA)
inline static bool is_muic_check_reset(struct max77854_muic_data *muic_data)
{
	bool ret;
...
inline static void set_muic_reset(struct max77854_muic_data *muic_data, bool val)
{
	mutex_lock(&muic_data->reset_mutex);
...
static irqreturn_t max77854_muic_irq(int irq, void *data)
{
	...
	if (muic_data->is_muic_ready == true)
		max77854_muic_detect_dev(muic_data, irq);
	else
...
do {									\
	ret = request_threaded_irq(_irq, NULL, max77854_muic_irq,	\
...
static int max77854_muic_irq_init(struct max77854_muic_data *muic_data)
{
	int ret = 0;
	...
	if (muic_data->mfd_pdata && (muic_data->mfd_pdata->irq_base > 0)) {
		...
		/* request MUIC IRQ */
		muic_data->irq_adc1k = irq_base + MAX77854_MUIC_IRQ_INT1_ADC1K;
		REQUEST_IRQ(muic_data->irq_adc1k, muic_data, "muic-adc1k");

		muic_data->irq_adcerr = irq_base + MAX77854_MUIC_IRQ_INT1_ADCERR;
		REQUEST_IRQ(muic_data->irq_adcerr, muic_data, "muic-adcerr");

		muic_data->irq_adc = irq_base + MAX77854_MUIC_IRQ_INT1_ADC;
		REQUEST_IRQ(muic_data->irq_adc, muic_data, "muic-adc");

		muic_data->irq_chgtyp = irq_base + MAX77854_MUIC_IRQ_INT2_CHGTYP;
		REQUEST_IRQ(muic_data->irq_chgtyp, muic_data, "muic-chgtyp");

		muic_data->irq_vbvolt = irq_base + MAX77854_MUIC_IRQ_INT2_VBVOLT;
...
static void max77854_muic_free_irqs(struct max77854_muic_data *muic_data)
{
	pr_info("%s:%s\n", MUIC_DEV_NAME, __func__);
...
#if defined(CONFIG_MUIC_MAX77854_RESET_WA)
static void max77854_muic_free_reset_irq(struct max77854_muic_data *muic_data)
{
	pr_info("%s:%s\n", MUIC_DEV_NAME, __func__);
...
#endif /* CONFIG_MUIC_MAX77854_RESET_WA */
...
static void max77854_muic_init_detect(struct max77854_muic_data *muic_data)
{
	pr_info("%s:%s\n", MUIC_DEV_NAME, __func__);
	...
	max77854_muic_detect_dev(muic_data, -1);

	mutex_unlock(&muic_data->muic_mutex);
}

#if defined(CONFIG_OF)
static int of_max77854_muic_dt(struct max77854_muic_data *muic_data)
{
...
static void max77854_muic_clear_interrupt(struct max77854_muic_data *muic_data)
{
	struct i2c_client *i2c = muic_data->i2c;
	...
	ret = max77854_read_reg(i2c, MAX77854_MUIC_REG_INT1, &interrupt1);
	if (ret)
		pr_err("%s:%s fail to read muic INT1 reg(%d)\n", MUIC_DEV_NAME, __func__, ret);

	ret = max77854_read_reg(i2c, MAX77854_MUIC_REG_INT2, &interrupt2);
	if (ret)
		pr_err("%s:%s fail to read muic INT2 reg(%d)\n", MUIC_DEV_NAME, __func__, ret);

	ret = max77854_read_reg(i2c, MAX77854_MUIC_REG_INT3, &interrupt3);
	if (ret)
...
static int max77854_muic_init_regs(struct max77854_muic_data *muic_data)
{
	int ret;

	pr_info("%s:%s\n", MUIC_DEV_NAME, __func__);

	max77854_muic_clear_interrupt(muic_data);

	/* Set ADC debounce time: 25ms */
	max77854_muic_set_adcdbset(muic_data, 2);
...
#if defined(CONFIG_HV_MUIC_MAX77854_AFC)
	ret = max77854_afc_muic_irq_init(muic_data);
	if (ret < 0) {
	...
	ret = max77854_muic_irq_init(muic_data);
	if (ret < 0) {
		pr_err("%s:%s Failed to initialize MUIC irq:%d\n", MUIC_DEV_NAME,
				__func__, ret);
		max77854_muic_free_irqs(muic_data);
	}
...
static irqreturn_t muic_reset_irq_thread(int irq, void* data)
{
	...
	max77854_bulk_read(muic_data->i2c, MAX77854_MUIC_REG_INTMASK1, 3,
				muic_irq_mask);
	...
	if ((reset_val == muic_irq_mask[0])
		...
		max77854_muic_handle_detach(muic_data);

#if defined(CONFIG_HV_MUIC_MAX77854_AFC)
		max77854_hv_muic_remove_wo_free_irq(muic_data);
		max77854_hv_muic_reset_hvcontrol_reg(muic_data);
#endif /* CONFIG_HV_MUIC_MAX77854_AFC */
		mutex_unlock(&muic_data->muic_mutex);
		...
		mutex_lock(&muic_data->muic_mutex);
		max77854_muic_init_regs(muic_data);
...
static int max77854_muic_reset_irq_init(struct max77854_muic_data *muic_data)
{
...
static int max77854_muic_probe(struct platform_device *pdev)
{
	struct max77854_dev *max77854 = dev_get_drvdata(pdev->dev.parent);
	struct max77854_platform_data *mfd_pdata = dev_get_platdata(max77854->dev);
	struct max77854_muic_data *muic_data;
	int ret = 0;

	pr_info("%s:%s\n", MUIC_DEV_NAME, __func__);

	muic_data = kzalloc(sizeof(struct max77854_muic_data), GFP_KERNEL);
	if (!muic_data) {
	...
	if (!mfd_pdata) {
		pr_err("%s: failed to get max77854 mfd platform data\n", __func__);
		ret = -ENOMEM;
		goto err_kfree;
	}

#if defined(CONFIG_OF)
	ret = of_max77854_muic_dt(muic_data);
	if (ret < 0) {
		pr_err("%s:%s not found muic dt! ret[%d]\n", MUIC_DEV_NAME, __func__, ret);
	}

#if defined(CONFIG_HV_MUIC_MAX77854_AFC)
	ret = of_max77854_hv_muic_dt(muic_data);
	if (ret < 0) {
		pr_err("%s:%s not found muic dt! ret[%d]\n", MUIC_DEV_NAME, __func__, ret);
	}
#endif /* CONFIG_HV_MUIC_MAX77854_AFC */
...
#else
	muic_data->ignore_adcerr = false;
#endif /* CONFIG_MUIC_MAX77854_IGNORE_ADCERR_WA */
	...
	/* create sysfs group */
	ret = sysfs_create_group(&switch_device->kobj, &max77854_muic_group);
	if (ret) {
	...
	ret = max77854_muic_init_regs(muic_data);
	if (ret < 0) {
		pr_err("%s:%s Failed to initialize MUIC irq:%d\n",
				MUIC_DEV_NAME, __func__, ret);
		goto fail_init_irq;
...
#if defined(CONFIG_HV_MUIC_MAX77854_AFC)
	max77854_hv_muic_initialize(muic_data);
#endif

#if defined(CONFIG_MUIC_MAX77854_RESET_WA)
	ret = max77854_muic_reset_irq_init(muic_data);
	if (ret < 0) {
	...
	max77854_hv_muic_init_check_dpdnvden(muic_data);
#endif /* CONFIG_HV_MUIC_MAX77854_AFC */

	/* initial cable detection */
	max77854_muic_init_detect(muic_data);

#if defined(CONFIG_HV_MUIC_MAX77854_AFC)
	/* initial hv cable detection */
	if (muic_data->is_afc_muic_ready)
		max77854_hv_muic_init_detect(muic_data);
#endif
...
fail_init_irq:
	...
	sysfs_remove_group(&switch_device->kobj, &max77854_muic_group);
fail_sysfs_create:
	mutex_unlock(&muic_data->muic_mutex);
fail:
...
static int max77854_muic_remove(struct platform_device *pdev)
{
	struct max77854_muic_data *muic_data = platform_get_drvdata(pdev);
...
#if defined(CONFIG_HV_MUIC_MAX77854_AFC)
		max77854_hv_muic_remove(muic_data);
#endif /* CONFIG_HV_MUIC_MAX77854_AFC */

#if defined(CONFIG_MUIC_MAX77854_RESET_WA)
		max77854_muic_free_reset_irq(muic_data);
#endif
		max77854_muic_free_irqs(muic_data);
...
void max77854_muic_shutdown(struct device *dev)
{
	struct max77854_muic_data *muic_data = dev_get_drvdata(dev);
	...
	max77854 = i2c_get_clientdata(i2c);
	pr_info("%s:%s max77854->i2c_lock.count.counter=%d\n", MUIC_DEV_NAME,
		__func__, max77854->i2c_lock.count.counter);

	ret = max77854_read_reg(i2c, MAX77854_MUIC_REG_CTRL3, &val);
	if (ret < 0)
...
static struct platform_driver max77854_muic_driver = {
	.driver		= {
		.name	= MUIC_DEV_NAME,
		.owner	= THIS_MODULE,
		.shutdown = max77854_muic_shutdown,
	},
	.probe		= max77854_muic_probe,
	.remove		= max77854_muic_remove,
};

static int __init max77854_muic_init(void)
{
	pr_info("%s:%s\n", MUIC_DEV_NAME, __func__);
	return platform_driver_register(&max77854_muic_driver);
}
module_init(max77854_muic_init);

static void __exit max77854_muic_exit(void)
{
	pr_info("%s:%s\n", MUIC_DEV_NAME, __func__);
	platform_driver_unregister(&max77854_muic_driver);
}
module_exit(max77854_muic_exit);

MODULE_DESCRIPTION("Maxim MAX77854 MUIC driver");
