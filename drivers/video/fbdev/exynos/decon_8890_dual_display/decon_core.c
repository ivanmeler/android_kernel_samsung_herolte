	 ...
	 struct decon_window_regs *win_regs, enum decon_idma_type idma_type)
{
	...
	decon_info("DMATYPE_%d@ SRC:(%d,%d) %dx%d  DST:(%d,%d) %dx%d\n",
			idma_type,
		...
		struct decon_win_config *win_config, struct decon_reg_data *regs)
{
	...
	if ((decon->id == 1) && (decon->pdata->out_type == DECON_OUT_DSI)) {
		decon_info("src info : %d %d %d %d %d %d \n",
			win_config->src.x, win_config->src.y, win_config->src.w, win_config->src.h, win_config->src.f_w, win_config->src.f_h);
		decon_info("dst info : %d %d %d %d %d %d \n",
			win_config->dst.x, win_config->dst.y, win_config->dst.w, win_config->dst.h, win_config->dst.f_w, win_config->dst.f_h);

		win_config->dst.x = 0;
		win_config->dst.y = 0;
		win_config->dst.w = 128;
		win_config->dst.h = 432;

		win_config->src.x = 0;
		win_config->src.y = 0;
		win_config->src.w = 128;
		win_config->src.h = 432;
...
static int decon_probe(struct platform_device *pdev)
{
	...
	if (decon->pdata->out_type == DECON_OUT_DSI) {
		...
		decon_reg_set_window_control(decon->id, win_idx, &win_regs, init_decon_color_map);
