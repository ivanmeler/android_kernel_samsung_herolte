...
// it is same as s6e3ha5_wqhd_aid_dimming.h
#define ELVSS_OFFSET_PARA (1)
#define ELVSS_OFFSET_OTP (0)
...
static void hf4_dsim_panel_set_elvss(struct dsim_device *dsim)
{
	u8 *elvss = NULL;
...
// it is first in HA5
static char dsim_panel_get_dim_elvssoffset(struct dsim_device *dsim)
{
	char retVal = 0x00;
	struct panel_private* panel = &(dsim->priv);
	struct SmtDimInfo *dimming_info = (struct SmtDimInfo *)panel->dim_info;
	...
	if(UNDER_MINUS_15(panel->temperature)) {
		retVal = dimming_info[panel->br_index].elvss_offset[DIM_UNDER_MINUS_FIFTEEN];
		pr_info("%s under -20 %d %d\n",
			__func__, retVal, dimming_info[panel->br_index].elvss_offset[DIM_UNDER_MINUS_FIFTEEN]);
	} else if(UNDER_0(panel->temperature)) {
		retVal = dimming_info[panel->br_index].elvss_offset[DIM_UNDER_ZERO];
		pr_info("%s under 0 %d %d\n",
			__func__, retVal, dimming_info[panel->br_index].elvss_offset[DIM_UNDER_ZERO]);
	} else {
		retVal = dimming_info[panel->br_index].elvss_offset[DIM_OVER_ZERO];
		pr_info("%s over 0 %d %d\n",
			__func__, retVal, dimming_info[panel->br_index].elvss_offset[DIM_OVER_ZERO]);
	}
...
static void ha5_dsim_panel_set_elvss(struct dsim_device *dsim)
{
	u8 *elvss = NULL;
	char value;
	unsigned char SEQ_ELVSS[ELVSS_LEN_MAX + 1] = {0, };
	...
	memcpy(&SEQ_ELVSS[panel->elvss_start_offset], elvss, 2);	// B5 reg : [0]=reg address(b5), [2]=MPS, [3]=ELVSS dim offset

	value = dsim_panel_get_elvssoffset(dsim);		// B5 reg : [24]=elvss_offset

	if (value == ELVSS_OFFSET_PARA) {
		SEQ_ELVSS[panel->elvss_temperature_offset] = SEQ_ELVSS[panel->elvss_temperature_offset + 1];	// B5 reg [24] = [25]

	}
	pr_info("%s %x %x %d\n", __func__, SEQ_ELVSS[panel->elvss_temperature_offset], SEQ_ELVSS[panel->elvss_temperature_offset + 1], value);

	if( dsim->priv.panel_type==LCD_TYPE_S6E3HA5_WQHD ) {
		value = dsim_panel_get_dim_elvssoffset(dsim);
		SEQ_ELVSS[panel->elvss_start_offset + 1] += value;	// B5 reg :  [3]=ELVSS
	}
...
void dsim_panel_set_elvss(struct dsim_device *dsim)
{
	...
	case LCD_TYPE_S6E3HA5_WQHD:
		ha5_dsim_panel_set_elvss( dsim );
		break;
	default:
		hf4_dsim_panel_set_elvss( dsim );
		break;
	}
}
...
static int dsim_panel_hbm_ctrl(struct dsim_device *dsim, bool is_hbm)
{
	int ret = 0;

	dsim_info( "%s : hbm %d\n", __func__, is_hbm );
	if( is_hbm ) {
		ret=dsim_write_hl_data(dsim, S6E3HA5_SEQ_HBM_OPR, ARRAY_SIZE(S6E3HA5_SEQ_HBM_OPR));
		if (ret < 0)	dsim_err("%s : fail to write S6E3HA5_SEQ_HBM_ON on command.\n", __func__);

		ret=dsim_write_hl_data(dsim, S6E3HA5_SEQ_HBM_ON, ARRAY_SIZE(S6E3HA5_SEQ_HBM_ON));
		if (ret < 0)	dsim_err("%s : fail to write S6E3HA5_SEQ_HBM_ON on command.\n", __func__);
	} else {
		ret=dsim_write_hl_data(dsim, S6E3HA5_SEQ_HBM_OFF, ARRAY_SIZE(S6E3HA5_SEQ_HBM_OFF));
		if (ret < 0)	dsim_err("%s : fail to write S6E3HA5_SEQ_HBM_ON on command.\n", __func__);
	}
...
static int low_level_set_brightness_for_hmt(struct dsim_device *dsim ,int force)
{
	...
	dsim_panel_set_vint(dsim, force);
	if (dsim_write_hl_data(dsim, HA5_IRC_off, ARRAY_SIZE(HA5_IRC_off)) < 0)
		dsim_err("%s : failed to write HA5_A3_IRC_off \n", __func__);
