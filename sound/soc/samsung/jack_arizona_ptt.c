...
static ssize_t earjack_ptt_state_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct arizona_extcon_info *info = dev_get_drvdata(dev);
...
static ssize_t earjack_ptt_state_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	pr_info("%s : operate nothing\n", __func__);
...
static DEVICE_ATTR(ptt_state, S_IRUGO | S_IWUSR | S_IWGRP,
		   earjack_ptt_state_show, earjack_ptt_state_store);
...
static void create_jack_devices(struct arizona_extcon_info *info)
{
	...
	if (device_create_file(jack_dev, &dev_attr_ptt_state) < 0)
		pr_err("Failed to create device file (%s)!\n",
			dev_attr_ptt_state.attr.name);
